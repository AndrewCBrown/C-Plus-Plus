...
void noobChecker(const string player)
{
	...
	else //If they entered 'y' then we explain the game rules here
	{
		...
		cout<<"You will not know the what the colors are but you will be given a certain number of turns to figure it out."<<endl;
		cout<<"Each round you get to guess the sequence and you will get feedback on your guess."<<endl;
		cout<<"For every 'W' you get, that represents that you guessed one of the colors in the sequence but you had it in the wrong spot."<<endl;
		cout<<"For every 'B' you get, that represents that you guessed the correct color in the correct spot."<<endl;
		cout<<"However, the 'W's and 'B's will not be in an order relating to your guess, so if you guessed Red Yellow Green,\nand the sequence was Orange Yellow Silver, you would get one 'B' but you won't know which of your guesses was the correct one."<<endl;
		...
		cout<<"=--------------------------------------------="<<endl;
		cout<<"= 1:  'R' 'O' 'Y' 'G' 'B' 'P'  |     WWWWW   ="<<endl;
		cout<<"= 2:  'G' 'Y' 'O' 'R' 'T' 'S'  |      WWWW   ="<<endl;
		cout<<"= 3:  'B' 'B' 'R' 'O' 'Y' 'G'  |      WWBB   ="<<endl;
		cout<<"= 4:  'R' 'R' 'R' 'R' 'R' 'R'  |         B   ="<<endl;
		cout<<"= 5:  'Y' 'Y' 'Y' 'Y' 'Y' 'Y'  |             ="<<endl;
		cout<<"= 6:  'P' 'B' 'R' 'O' 'G' 'S'  |     WWWWB   ="<<endl;
		cout<<"= 7:  'B' 'T' 'O' 'R' 'P' 'G'  |    WWWWBB   ="<<endl;
		cout<<"= 8:  'B' 'T' 'O' 'P' 'R' 'G'  |    WWWWWB   ="<<endl;
		cout<<"= 9:  'B' 'S' 'S' 'R' 'P' 'S'  |       WBB   ="<<endl;
		cout<<"=10:  'B' 'G' 'R' 'O' 'P' 'T'  |    WWBBBB   ="<<endl;
		cout<<"=11:  'B' 'R' 'G' 'O' 'P' 'T'  |    BBBBBB   ="<<endl;
		cout<<"=12:  ' ' ' ' ' ' ' ' ' ' ' '  |             ="<<endl;
		cout<<"=13:  ' ' ' ' ' ' ' ' ' ' ' '  |             ="<<endl;
		cout<<"=14:  ' ' ' ' ' ' ' ' ' ' ' '  |             ="<<endl;
		cout<<"=15:  ' ' ' ' ' ' ' ' ' ' ' '  |             ="<<endl;
...
void printGameBoard(const unsigned int round, const size_t guess_array_size, const unsigned int currentRound, const char* const guess, const string difficulty, const unsigned int partial_correct, const unsigned int full_correct, const bool won, const char* const key)
{
	...
	for (unsigned int i=0; i < guess_array_size; i++)
	{
		...
		{
			feedback[currentRound][i] = 'W';
		}
		else
		{
			feedback[currentRound][i] = 'B';
		}
	}
